...
class Welcome extends CI_Controller
{
	...
	public function create_box()
	{
		...
		$role_management_data = $this->db->query("SELECT *   FROM `box` WHERE created_time >= '$from_date' AND created_time <=  '$to_date' ORDER BY id DESC  ");
	...
	public function create_invoice()
	{
		...
		$role_management_data = $this->db->query("SELECT *   FROM `invoice` WHERE created_time >= '$from_date' AND created_time <=  '$to_date' ORDER BY id DESC  ");
	...
	public function view_packing()
	{
		...
		$role_management_data = $this->db->query("SELECT *   FROM `packing` WHERE created_time >= '$from_date' AND created_time <=  '$to_date' ORDER BY id DESC  ");
	...
	public function view_packing_by_id()
	{
		...
		// $data['customers_part_type'] = $this->Crud->read_data("customer_part_type");
		// $data['customers'] = $this->Crud->read_data("customer");
		$data['packing'] = $this->Crud->get_data_by_id("packing", $id, "id");
	...
	public function view_batch_code()
	{
		$to_date = $this->input->post('to_date');
		$from_date = $this->input->post('from_date');

		if ($this->input->post('batch_code')) {
			$data['batch_code_document'] = $this->Crud->get_data_by_id('batch_code_document', $this->input->post('batch_code'), 'batch_code');
			$role_management_data = $this->db->query("SELECT *  FROM `packing` WHERE batch_code = '" . $this->input->post('batch_code') . "' ");


			// echo "SELECT *   FROM `packing` WHERE created_time >= $from_date AND created_date <=  $to_date";
			$data['packing'] = $role_management_data->result();
			
			$data['batch_code_selected'] = $this->input->post('batch_code');
		} else {
			$from_date = $this->current_date;
			$to_date = $this->current_date;
			$data['batch_code_document'] = "";
			$data['packing'] = "";
			$data['batch_code_selected'] = "";
		}
		...
		$batch_code = $this->db->query("SELECT DISTINCT batch_code    FROM `packing`  ");

		// echo "SELECT *   FROM `box` WHERE created_time <= '$from_date' AND created_time >=  '$to_date'";
		$data['batch_code'] = $batch_code->result();
		// $customer_id = $this->uri->segment('2');
		// $data['customer_id'] = $this->uri->segment('2');
		...
		// print_r($data['customer_part_list']);
		$this->load->view('header');
		$this->load->view('view_batch_code', $data);
		$this->load->view('footer');
	}
	public function create_packing_data()
	{
		...
		$data['partss'] = $this->Crud->get_data_by_id("parts", $scan_code, "part_code");
		$part_id = $data['partss'][0]->id ;
		//$part_id = $this->input->post('part_id');
		$part_qty = $this->input->post('part_qty');
		$batch_code = $this->input->post('batch_code');

		$packing = $this->Crud->read_data("packing");
		// $batch_code = $this->Crud->read_data("batch_code");
		if ($packing) {
			$count = (int)($packing[0]->barcode) + 1;
		} else {
			$count = 0;
		}
	...
	public function create_packing_data_bulk()
	{
		...
		$data['partss'] = $this->Crud->get_data_by_id("parts", $scan_code, "part_code");
		//print_r($data['partss'][0]->id );die();
		$part_id1 = $data['partss'][0]->id ;
	...
	public function upload_batch_document()
	{
		$batch_code = $this->input->post('batch_code');
		...
		if (false) {
			...
			$result = $this->Crud->insert_data("batch_code_document", $data);
			if ($result) {
				echo "<script>alert('Added Sucessfully');document.location='" . $_SERVER['HTTP_REFERER'] . "'</script>";
			} else {
				echo "<script>alert('Unable to Add');document.location='" . $_SERVER['HTTP_REFERER'] . "'</script>";

...
class Newcontroller extends CI_Controller
{
	...
	public function generate_new_po()
	{
		...
		if ($data['new_po']) {
			$count = count($data['new_po']);
		} else {
			$count = 0;
		}


		$po_number = $this->year . '' . $this->month . '' . $count;
	...
	public function add_planning_data()
	{
		...
		if ($bom_data) {
			foreach ($bom_data as $b) {
				$child_part_data = $this->Crud->get_data_by_id("child_part_master", $b->child_part_id, "id");
	...
	public function add_invoice_number()
	{
		...
		if ($check != 0) {
			echo "<script>alert('Error 403 : Invoice Number  Already Exists To This PO Number , Enter Different Invoice Number ');document.location='" . $_SERVER['HTTP_REFERER'] . "'</script>";
		} else {
			...
			$grn_number = $this->year . "-" . $this->month . "-" . $inwarding_count;
